#include "dynamic_array.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <assert.h>
#include <math.h>

int num_arrays = 0;
DynamicArray** arraysAllocated = NULL;

/* private functions *********************************************************/

/* Position in the buffer of the array element at position index */
static int index_to_offset ( const DynamicArray * da, int index ) {
    return index + da->origin;
}

/* Position of the element at buffer position 'offset' */
static int offset_to_index ( const DynamicArray * da, int offset ) {
    return offset - da->origin;
}

/* Non-zero if and only if offset lies ouside the buffer */
static int out_of_buffer ( DynamicArray * da, int offset ) {
    return offset < 0 || offset >= da->capacity;
}

/* Makes a new buffer that is twice the size of the old buffer,
   copies the old information into the new buffer, and deletes
   the old buffer */
static void extend_buffer ( DynamicArray * da ) {

    double * temp = (double *) calloc ( 2 * da->capacity, sizeof(double) );
    int new_origin = da->capacity - (da->end - da->origin)/2,
           new_end = new_origin + (da->end - da->origin);

    for ( int i=0; i<DynamicArray_size(da); i++ ) {
        temp[new_origin+i] = DynamicArray_get(da,i);
    }

    free(da->buffer);
    da->buffer = temp;

    da->capacity = 2 * da->capacity;
    da->origin = new_origin;
    da->end = new_end;

    return;

}

/* public functions **********************************************************/

DynamicArray* DynamicArray_new(void) {
    DynamicArray * da = (DynamicArray *) malloc(sizeof(DynamicArray));
    da->capacity = DYNAMIC_ARRAY_INITIAL_CAPACITY;
    da->buffer = (double *) calloc ( da->capacity, sizeof(double) );
    da->origin = da->capacity / 2;
    da->end = da->origin;
    DynamicArray** new_arraysAllocated = (DynamicArray**)realloc(arraysAllocated, (num_arrays + 1) * sizeof(DynamicArray*));
    arraysAllocated = new_arraysAllocated;
    arraysAllocated[num_arrays] = da;
    num_arrays++;
    return da;
}

void remove_from_arrays_allocated(DynamicArray* da) {
    int found = 0;
    for (int idx = 0; idx < num_arrays; idx++) {
        if (arraysAllocated[idx] == da) {
            found = 1;
        }
        if (found && idx < num_arrays - 1) {
            arraysAllocated[idx] = arraysAllocated[idx + 1];
        }
    }
    if (found) {
        num_arrays--;  // Decrement the num_arrays counter
        arraysAllocated = (DynamicArray**)realloc(arraysAllocated, num_arrays * sizeof(DynamicArray*));
    }
}

void DynamicArray_destroy(DynamicArray * da) {
    assert(da->buffer != NULL);
    remove_from_arrays_allocated(da);
    free(da->buffer);
    da->buffer = NULL;
    return;
}

int DynamicArray_size(const DynamicArray * da) {
    assert(da->buffer != NULL);
    return da->end - da->origin;
}

char * DynamicArray_to_string(const DynamicArray * da) {
    assert(da->buffer != NULL);
    char * str = (char *) calloc (20,DynamicArray_size(da)),
         temp[20];
    int j = 1;
    str[0] = '[';
    for ( int i=0; i < DynamicArray_size(da); i++ ) {
        if ( DynamicArray_get(da,i) == 0 ) {
            snprintf ( temp, 20, "0" );
        } else {
            snprintf ( temp, 20, "%.5lf", DynamicArray_get(da,i) ); 
        }
        if ( i < DynamicArray_size(da) - 1 ) {
            sprintf( str + j, "%s,", temp);
            j += strlen(temp) + 1;
        } else {
            sprintf( str + j, "%s", temp);
            j += strlen(temp);
        }

    }
    str[j] = ']';
    return str;
}

void DynamicArray_print_debug_info(const DynamicArray * da) {

    char * s = DynamicArray_to_string(da);
    printf ( "  %s\n", s);
    printf ( "  capacity: %d\n  origin: %d\n  end: %d\n  size: %d\n\n",
      da->capacity, 
      da->origin, 
      da->end,
      DynamicArray_size(da));

    free(s);

}

void DynamicArray_set(DynamicArray * da, int index, double value) {
    assert(da->buffer != NULL);
    assert ( index >= 0 );
    while ( out_of_buffer(da, index_to_offset(da, index) ) ) {
        extend_buffer(da);
    }
    da->buffer[index_to_offset(da, index)] = value;
    if ( index >= DynamicArray_size(da) ) {
        da->end = index_to_offset(da,index+1);
    }

}

double DynamicArray_get(const DynamicArray * da, int index) {
    assert(da->buffer != NULL);
    assert ( index >= 0 );
    if ( index >= DynamicArray_size(da) ) {
        return 0;
    } else {
        return da->buffer[index_to_offset(da,index)];
    }
}

void DynamicArray_push(DynamicArray * da, double value ) {
    DynamicArray_set(da, DynamicArray_size(da), value );
}

void DynamicArray_push_front(DynamicArray * da, double value) {
    assert(da->buffer != NULL);
    while ( da->origin == 0 ) {
        extend_buffer(da);
    }
    da->origin--;
    DynamicArray_set(da,0,value);
}

double DynamicArray_pop(DynamicArray * da) {
    //ssert(DynamicArray_size(da) > 0);
    assert(da != NULL || da->end > da->origin);
    double value = DynamicArray_get(da, DynamicArray_size(da)-1);
    DynamicArray_set(da, DynamicArray_size(da)-1, 0.0);
    da->end--;
    return value;
}

double DynamicArray_pop_front(DynamicArray * da) {
    assert(da->buffer != NULL ||  da->origin < da->end);
    assert(DynamicArray_size(da) > 0);
    double value = DynamicArray_get(da, 0);
    da->origin++;
    return value;    
}

DynamicArray * DynamicArray_map(const DynamicArray * da, double (*f) (double)) {
    assert(da->buffer != NULL);
    DynamicArray * result = DynamicArray_new();
    for ( int i=0; i<DynamicArray_size(da); i++ ) {
        DynamicArray_set(result, i, f(DynamicArray_get(da, i)));
    }
    return result;
}

DynamicArray * DynamicArray_subarray(DynamicArray * da, int a, int b) {

  assert(da->buffer != NULL);
  assert(b >= a);

  DynamicArray * result = DynamicArray_new();

  for (int i=a; i<b; i++) {
      DynamicArray_push(result,DynamicArray_get(da, i));
  }

  return result;

}

double DynamicArray_min(const DynamicArray* da) {
    assert(da->buffer != NULL);
    double min = INFINITY;
    int index = 0;
    while (index < DynamicArray_size(da)) {
        double value = DynamicArray_get(da, index);
        min = (min > value) ? value : min;
        index++;
    }
    return min;
}

double DynamicArray_max(const DynamicArray* da) {
    assert(da->buffer != NULL);
    double max = -INFINITY;
    int index = 0;
    while (index < DynamicArray_size(da)) {
        double value = DynamicArray_get(da, index);
        max = (max < value) ? value : max;
        index++;
    }
    return max;
}

double DynamicArray_mean(const DynamicArray* da) {
    assert(da->buffer != NULL);
    double total = 0;
    for (int idx = 0; idx < DynamicArray_size(da); idx++) {
        total += DynamicArray_get(da, idx);
    }
    return total / DynamicArray_size(da);
}

void DynamicArray_bubblesort(DynamicArray* da) {
    assert(da->buffer != NULL);
    for (int i = 0; i < DynamicArray_size(da) - 1; i++) {
        for (int j = 0; j < DynamicArray_size(da) - i - 1; j++) {
            if (DynamicArray_get(da, j) > DynamicArray_get(da, j + 1)) {
                double temp = DynamicArray_get(da, j);
                DynamicArray_set(da, j, DynamicArray_get(da, j + 1));
                DynamicArray_set(da, j + 1, temp);
            }
        }
    }
}

DynamicArray * DynamicArray_copy(const DynamicArray * da) {
    assert(da->buffer != NULL);
    DynamicArray * da_copy = DynamicArray_new();
    for (int idx = 0; idx < DynamicArray_size(da); idx++) {
        DynamicArray_set(da_copy, idx, DynamicArray_get(da, idx));
    }
    return da_copy;
}

double DynamicArray_median(const DynamicArray* da) {
    assert(da->buffer != NULL);
    // We don't want to alter the referenced array here, so perform a deep copy.
    DynamicArray * da_copy = DynamicArray_copy(da);
    DynamicArray_bubblesort(da_copy);
    if ((DynamicArray_size(da) & 1) == 0) {
        double median = DynamicArray_get(da_copy, (DynamicArray_size(da) / 2) - 1) + DynamicArray_get(da_copy, DynamicArray_size(da) / 2) / 2;
        DynamicArray_destroy(da_copy);
        return median;
    }
    else {
        double median = DynamicArray_get(da, DynamicArray_size(da) / 2);
        DynamicArray_destroy(da_copy);
        return median;
    }
}

double DynamicArray_sum(const DynamicArray* da) {
    int index = 0;
    double sum = 0;
    while (index < DynamicArray_size(da)) {
        sum += DynamicArray_get(da, index);
        index++;
    }
    return sum;
}

double DynamicArray_first(const DynamicArray * da) {
    assert(da->buffer != NULL);
    return *(da->buffer + da->origin);
}

double DynamicArray_last(const DynamicArray * da) {
    assert(da->buffer != NULL);
    return *(da->buffer + da->end - 1);
}

DynamicArray * DynamicArray_range(double begin, double end, double step) {
    assert(end > begin);
    assert(step > 0);
    DynamicArray * da = DynamicArray_new();
    int idx = 0;
    double value = begin;
    while (value < end) {
        DynamicArray_set(da, idx, value);
        value += step;
        idx++;
    }
    return da;
}

DynamicArray * DynamicArray_concat(const DynamicArray * a, const DynamicArray * b) {
    DynamicArray *concatenated = DynamicArray_new();
    int size_a = DynamicArray_size(a);
    int size_b = DynamicArray_size(b);
    for (int idx = 0; idx < size_a; idx++) {
        DynamicArray_set(concatenated, idx, DynamicArray_get(a,idx));
    }
    for (int idx = 0; idx < size_b ; idx++) {
        DynamicArray_set(concatenated, size_a + idx, DynamicArray_get(b,idx));
    }
    return concatenated;
}

DynamicArray * DynamicArray_take(DynamicArray * da, int pointer) {
    int overflow = 0;
    DynamicArray * da_new = DynamicArray_new();
    if(abs(pointer) > DynamicArray_size(da)) {
        overflow = abs(pointer) - DynamicArray_size(da);
        if(pointer < 0) {
            pointer = -DynamicArray_size(da);
        }
        else {
             pointer = DynamicArray_size(da);
        }
    }
    if(pointer < 0) {
        for (int idx = 0; idx < abs(pointer); idx++) {
            DynamicArray_set(da_new, idx, DynamicArray_get(da, DynamicArray_size(da) + pointer + idx));
        }
        if (overflow > 0) {
            DynamicArray * zeros = DynamicArray_new();
            for (int idx = 0; idx < overflow; idx++) {
                DynamicArray_set(zeros, idx, 0);
            }
            da_new = DynamicArray_concat(da_new, zeros);
            DynamicArray_destroy(zeros);
        }
        return da_new;
    }
    else if (pointer == DynamicArray_size(da)) {
        if (overflow > 0) {
            DynamicArray * zeros = DynamicArray_new();
            for (int idx = 0; idx < overflow; idx++) {
                DynamicArray_set(zeros, idx, 0);
            }
            da_new = DynamicArray_concat(DynamicArray_copy(da), zeros);
            DynamicArray_destroy(zeros);
        }
        else {
            da_new = DynamicArray_copy(da);
        }
        return da_new;
    }
    else {
        return DynamicArray_subarray(da, 0, pointer);
    }
}

int DynamicArray_num_arrays() {
    return num_arrays;
}

void DynamicArray_destroy_all() {
    for (int idx = 0; idx < num_arrays; idx++) {
        arraysAllocated[idx]->buffer = NULL;
        free(arraysAllocated[idx]->buffer);
    }
    num_arrays = 0;
}

int DynamicArray_is_valid(const DynamicArray * da) {
    if (da->buffer == NULL) {
        return 0;
    }
    return 1;
}